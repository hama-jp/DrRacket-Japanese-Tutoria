...
「内側の式を値に置き換える」を繰り返すだけ。この **置換モデル(substitution model)** は Lisp の評価を追う基本ツールで、バグを追う時にも「いまどの置換が起きているか」を頭の中で描けると強いです。

### 引数の評価順も忘れないで

`(f a b c)` の形を評価するとき、`a`, `b`, `c` は **左から順に** 1 つずつ評価されます。これは引数の中に副作用があるときに効いてきます。

```text
> (define (log-and-return name v)
    (printf "~a => ~a~n" name v)
    v)
> (+ (log-and-return 'a 1) (log-and-return 'b 2) (log-and-return 'c 3))
a => 1
b => 2
c => 3
6
...
**宣言順どおりに** 副作用が出ています。一部の言語(古い C の一部仕様や一部の JavaScript エンジン)では引数の評価順が規定されていなかったり実装依存だったりしますが、Racket では保証されています。
...
`+` は **可変長引数**を取れます(他言語の演算子とはここが大きく違う)。「2 個しか足せない」制約がないので、`(apply + '(1 2 3 4))` のような組み合わせが非常にきれいに書けます。
...
`eval` はデータとしての式を **評価に戻す** 関数です。

- 第 1 引数: 評価したい **S式(データ)**
- 第 2 引数: どの名前空間で評価するか(`+` や `*` のような標準手続きを引くための辞書)

`(make-base-namespace)` は「`racket/base` の基本束縛を持ったまっさらな名前空間」を返す関数です。`eval` 単体で使うと「現在モジュールの名前空間」がデフォルトになりますが、それはテスト・ライブラリ用途で混乱しがちなので、練習ではこのように明示的に渡すのが分かりやすいです。

これは普段書く Racket コードでは **ほぼ使いません**(ほとんどのメタプログラミングはマクロで事足りる)。ただし「Lisp はコードとデータに壁がない」という感触を掴むには、一度触っておく価値のある実験です。
