...
普通の Web アプリでは、ユーザとのやり取りを「ページ A を返す」→「POST を受ける」→「ページ B を返す」というふうに **別々のハンドラ関数** として書きます。これをセッション ID やクッキーでつなぎ直すのがよくある方式です。

Racket の `send/suspend` では、次のような手順で同じことを **1 つの関数として** 書けます。

1. サーバが `send/suspend` を呼ぶ
2. その時点での「ここから先でするはずだった計算」を処理系が **凍結** し、一意の URL に保存
3. 凍結された URL を渡されたコールバック関数(上の `k-url`)が HTML を返す
4. ユーザがその URL にアクセス(ここでは form の送信先として埋め込み)
5. サーバが凍結を **解凍** して、`send/suspend` の次の行から再開する
6. `req2` に 2 回目のリクエストが入った状態で、残りの処理が進む

「`define req2 (send/suspend ...)` の右辺がページ A を作り、それが返ってきたときには左辺に `req2` が入っている」というのが不思議なポイントです。普通の関数ならユーザのクリックを待てないはずなのに、**継続** という仕組みのおかげで関数の途中で一時停止して再開できる。
...
- サーバメモリに凍結された継続が残るため、長時間運用では古い継続の掃除が必要(`stuffers` 機能で対応)
- 複数サーバでロードバランスするアプリには工夫が要る(継続をどこで凍結したか分散させる)
- ユーザが「戻る」を押すとどこに飛ぶかをきちんと設計しないと混乱する

学習用・小規模アプリ・同人プロジェクトでは強力で楽しい道具です。本格的な運用では `web-server/dispatch` + セッションという王道パターンも選択肢に入ります。継続の詳細は第 17 章で再訪します。
