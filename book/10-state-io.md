...
- `(parameter-name)` で現在値を取得
- `parameterize` ブロックの **中だけ** 値を差し替えられる
- ブロックを抜けると **元の値に自動で戻る**
- 複数スレッドから見ても安全(スレッドごとに独立した値を持つ)

`parameterize` は第 5 章の `let` の「動的」版と考えると理解が早いです。`let` は「ソースコード上でここから見える範囲で名前を束縛する」静的束縛ですが、`parameterize` は「この呼び出し階層の間だけ値を差し替える」動的束縛です。

### 自作パラメータ

`make-parameter` で自分のパラメータも作れます。

```text
> (define user-lang (make-parameter 'ja))
> (user-lang)
'ja
> (define (greet name)
    (case (user-lang)
      [(ja) (string-append "こんにちは、" name)]
      [(en) (string-append "hello, " name)]))
> (greet "Reki")
"こんにちは、Reki"
> (parameterize ([user-lang 'en])
    (greet "Reki"))
"hello, Reki"
> (user-lang)
'ja
...
関数 `greet` に `lang` 引数を毎回引き回さずに、呼び出し側が「この区間だけ英語にしたい」と宣言できる。

### 典型的な用途

- **ロギングの設定**: `(current-logger)` を差し替えて、テスト中だけ出力先を変える
- **乱数の再現性**: `(current-pseudo-random-generator)` を固定して決定的にする
- **I/O のリダイレクト**: `(current-output-port)` / `(current-input-port)` をテスト用の文字列ポートに
- **DSL のコンテキスト**: `user-lang` のように、いまの言語・時刻・ユーザなどを切り替える

**グローバル変数で済ませたくなった状況のほぼ全部** を、`parameterize` で安全に置き換えられます。特にテストで一時的に設定を変えたいときに威力を発揮します。
