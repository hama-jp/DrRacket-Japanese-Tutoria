...
- 同じ名前の `module+` ブロックは **何回でも書ける**。ファイル中で散らばっていても、最終的に 1 つのサブモジュールに統合される。関連のある関数の直下にテストを置けるのが強み。
- サブモジュールの親の束縛(`square` など)には **自動でアクセスできる**。`require` は要らない。ただし `rackunit` のような外部ライブラリは `module+ test` の中で `require` する。
- `raco test file.rkt` は既定で `test` という名前のサブモジュールだけを走らせる。
- `racket -t file.rkt -m` は `main` サブモジュールを走らせる。コマンドラインエントリポイントと、普通に `require` したときに走るトップレベルを分けるのに便利。
