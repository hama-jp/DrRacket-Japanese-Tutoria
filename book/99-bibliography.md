...
本書の第 2 版では、Racket の核心である「言語を作る言語」としての側面をさらに深く掘る予定です。具体的には次のような章立てを構想しています。

1. **マクロの理論と実装** — 衛生性の仕組み、束縛 / スコープ / 相の扱い、`syntax-parse` の構文クラスを自作
2. **自作 `#lang` 入門** — Beautiful Racket の流れを組んで、独自の小言語を書き下ろす
3. **リーダ拡張** — S式以外の構文を受け付けるパーサの書き方、`#%module-begin` の役割
4. **Racket のコンパイル過程** — `expand` / `compile` / `eval` の関係、Macro Stepper の読み方
5. **DSL ケーススタディ** — JSON パーサ、状態機械言語、テスト記述言語などの具体例

「マクロはとりあえず動いた」から「マクロ処理系を理解して設計できる」への橋渡しを目指します。
