...
`(module 名前 言語 本体...)` は Racket の中で「新しいモジュールをここに作る」と宣言する特殊フォームです。`#lang racket` という 1 行目は、Racket 処理系に「ファイル全体を `(module <ファイル名> racket ...)` という括りに入れた扱いで読んでね」とお願いする **マクロ的な省略記法** だと思ってください。

モジュールを意識する利点はこの先、次第に大きくなります。

- ファイル内で定義した関数や変数は、**モジュールの外から見ると「公開(`provide`)していないものは存在しない」** 扱いになる
- `(require "foo.rkt")` と書くと、その `foo.rkt` を 1 つのモジュールとして読み込み、公開されたものだけが名前解決で見える
- 同じファイル内にも **サブモジュール**(`module+ test` など)を埋め込んで、テストや実行エントリを同居できる

これらは第 11 章で本格的に扱いますが、「1 ファイル = 1 モジュール」という骨格だけ今のうちに頭に入れておくと、後で迷いません。
